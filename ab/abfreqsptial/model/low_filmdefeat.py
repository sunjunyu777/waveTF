...
class LowFiLM(nn.Module):
    ...
    def __init__(self, num_channels, k_size=7):
        ...
        # 瞬态分支：[frame_diff_gap, frame_diff_gmp] → logits_max
        self.conv_max = nn.Conv1d(2, 1, kernel_size=k_size, padding=pad, bias=True)
    ...
    def forward(self, x, low_freq_temporal):
        ...
        # 3. 瞬态分支（看变化）
        # 帧差 GAP+GMP 双描述：平均变化量 + 峰值变化量
        frame_diff = (x - low_freq_temporal).abs()   # [B, C, H, W]
        z_max = torch.stack([
            frame_diff.mean(dim=(2, 3)),   # 平均变化量
            frame_diff.amax(dim=(2, 3)),   # 峰值变化量
        ], dim=1)  # [B, 2, C]
