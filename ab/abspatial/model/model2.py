...
class AFF(nn.Module):
    """
    Attentional Feature Fusion (Dai et al., WACV 2021)
    
    将两路同形状特征 (t, s) 融合为单路：
        combined = t + s
        weight   = sigmoid(LocalAttn(combined) + GlobalAttn(combined))
        fused    = t * weight + s * (1 - weight)
    
    - LocalAttn: 1×1 conv 看通道局部上下文
    - GlobalAttn: GAP + 1×1 conv 看全局场景
    - 通道+空间双重自适应权重
    """
    def __init__(self, dim, reduction=4):
        super().__init__()
        inter = max(dim // reduction, 16)
        # 局部分支（保留空间）—— 输入 2C 拼接，保留两路差异信息
        self.local_att = nn.Sequential(
            nn.Conv2d(2 * dim, inter, 1),
            LayerNorm(inter, eps=1e-6, data_format="channels_first"),
            nn.GELU(),
            nn.Conv2d(inter, dim, 1),
    ...
    def forward(self, t, s):
        """
        Args:
            t, s: [B, C, H, W] 两路待融合特征（异构分支）
        Returns:
            fused: [B, C, H, W]
        """
        combined = torch.cat([t, s], dim=1)  # [B, 2C, H, W]
        weight = torch.sigmoid(self.local_att(combined) + self.global_att(combined))
        return t * weight + s * (1 - weight)
...
class VCC(nn.Module):
    def __init__(self, in_chans=3, out_chans=1, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], 
        ...
        # Stage3: 双分支融合（AFF）+ 投影
        self.fusion_s3 = AFF(dim=dims[2], reduction=4)           # 384 -> 384
        self.reduce_s3 = nn.Sequential(
            nn.Conv2d(dims[2], dims[2], kernel_size=1),          # 384 -> 384 投影
    ...
    def forward(self, frames):
        ...
        # 3.5) 高级两路融合：时频分支 + 频空分支 (已注释)
        # frequency_s4 = self.advanced_fusion_s4(
        #     z_tf=temporal_s4,           # 时频特征
        #     z_fs=spatial_s4_fused       # 频空特征
        # )  # [B, 768, H/32, W/32]

        # 3.6) 分别降维两个分支并上采样到 Stage3
        temporal_s4_reduced = self.reduce_s4(temporal_s4)  # [1, 384, H/32, W/32]
        spatial_s4_reduced = self.reduce_s4(spatial_s4_fused)  # [1, 384, H/32, W/32]
        temporal_s4_upsampled = F.interpolate(temporal_s4_reduced, size=frame2_stage3.shape[-2:], mode='bilinear', align_corners=False)  # [1, 384, H/16, W/16]
        spatial_s4_upsampled = F.interpolate(spatial_s4_reduced, size=frame2_stage3.shape[-2:], mode='bilinear', align_corners=False)  # [1, 384, H/16, W/16]
        
        # ===== Stage3: FiLM调制 + 双分支融合 =====
        # (low_t_s3, high_t_s3 已在前面通过 backbone 共享路径生成)

        # 4.2) 用低频相似度引导调制 temporal_s4_upsampled
        low_s3_fused = self.low_film_s3(
            x=temporal_s4_upsampled,    # 来自Stage4的时频分支
        ...
        spatial_s3_fused, density_s3 = self.spatial_density_s3(
            freq_feat=spatial_s4_upsampled,  # 来自Stage4的频空分支
            spatial_feat=frame2_stage3        # 空间特征（提供密度先验）
        )  # [1, 384, H/16, W/16], [1, 1, H/16, W/16]
        
        # 4.5) 双分支融合（AFF：local + global 双路 attention 决定加权）
        frequency_s3 = self.fusion_s3(temporal_s3, spatial_s3_fused)  # [B, 384, H/16, W/16]

        # 4.6) 投影并上采样到输出分辨率
        fused_s3 = self.reduce_s3(frequency_s3)  # [1, 384, H/16, W/16]
