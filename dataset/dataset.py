...
class Crowd(data.Dataset):
    def __init__(self, root_path, is_gray=False, method='train', frame_number=3,
                 crop_height=512, crop_width=512, roi_path=None, return_density=False):
    ...
    def __getitem__(self, item):
        ...
        if self.method == 'train' and 'venice' not in self.root_path:
            ...
            for q in range(item, item+self.frame_number):
                ...
                keypoint_list.append(keypoint)
                target = cv2.resize(h5_map, (h5_map.shape[1] // 32 * 4, h5_map.shape[0] // 32 * 4),
                                    interpolation=cv2.INTER_CUBIC) * (
                                (h5_map.shape[0] * h5_map.shape[1]) /
                                ((h5_map.shape[1] // 32 * 4) * (h5_map.shape[0] // 32 * 4)))
                if self.roi_path:
                    target = target * mask_resize
                target_list.append(torch.from_numpy(target.copy()).float().unsqueeze(0))

            if self.return_density:
                return torch.stack(img_list, dim=0), torch.tensor(keypoint_list), torch.stack(mask_list, dim=0), torch.stack(target_list, dim=0)
